...
* **[P0 · çökme] React Hooks kuralı ihlali — agency & restaurant dashboard layout.** `app/dashboard/agency/layout.tsx:23` ve `app/dashboard/restaurant/layout.tsx:23`: `useState(isMobileMenuOpen)` satır 22'deki `if (isLoading || !user) return null;` erken dönüşünden SONRA çağrılıyor. `isLoading` true→false geçişinde render'daki hook sayısı değiştiği için React "Rendered more hooks than during the previous render" hatasıyla çökebilir. `npm run lint` bu yüzden 2 error veriyor (F1-01 kaynaklı değil, önceden var). Düzeltme tek satırlık: `useState` çağrısını erken dönüşün üstüne taşı. Not: bu iki dosya önceki oturumdaki route-guard çalışmasından geliyor.
