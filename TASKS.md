...
### [x] F5-01 · Transfer (shuttle) müşteri akışı
...
**Notlar:**
- **[P0] Transfer overbooking yarışı düzeltildi (bu görevin ana bulgusu).** `_create_shuttle_booking` artık kontenjanı tur akışıyla (F2-02) birebir aynı desende, **`create()` anında koşullu atomik UPDATE** ile tutuyor (`booked_count__lte=F('max_capacity')-guests`); kaybeden istek 0 satır günceller ve 400 alır. Öncesinde yalnız salt-okunur `remaining` kontrolü + webhook'ta `+=` artış vardı → N eşzamanlı istek aynı değeri okuyup hepsi geçerek overbooking yapabiliyordu (para çoktan alınmışken). Bu, `## BULUNAN YENİ SORUNLAR` altında not düşülen "Transfer akışı F5-01 kapsamında; aynı düzeltme orada uygulanmalı" maddesinin karşılığıdır — **çözüldü**.
- Webhook `SUCCEEDED` artık transfer kontenjanına **dokunmuyor** (çift sayım kalktı); yalnız durumu `confirmed` yapıyor. Webhook `FAILED` ve `cancel` (pending+confirmed) artık `_release_capacity` ile türüne göre (tur/transfer) kontenjanı bırakıyor.
- Yeni `_release_shuttle_capacity` + `_release_capacity` dispatcher eklendi; `Booking.create` transfer dalına `guest_*` alanları eklendi (misafir/üyeliksiz alım pariteliği, F4-07 ile aynı).
- Frontend: `app/transfer/[id]` sahte "rezervasyon alındı" onayı + auth gate kaldırıldı; artık ortak `/checkout?type=shuttle&shuttleId=&date=&time=&guests=` akışına yönlendiriyor. `checkout` sayfasına transfer dalı eklendi (fetchShuttle, `shuttle_route_id`+`start_time` POST gövdesi, transfer sipariş özeti; tur-özel iptal politikası bloğu null-tour çökmesini önlemek için tur dalında kaldı). Acente `shuttles` paneline soft-delete (`DELETE /agency/shuttles/{id}/`) butonu eklendi — CRUD tamamlandı.
- **Testler:** `bookings/tests.py` — yeni `ShuttleCapacityReservationTestCase` (create-anı rezervasyon, overbooking reddi, fiyat server-side, pending iptal bırakma, başarısız ödeme bırakma); `WebhookIdempotencyTestCase::test_shuttle_webhook_increments_quota_once` → `test_shuttle_webhook_never_touches_quota` olarak güncellendi (artık webhook kontenjana dokunmadığını doğruluyor). 78 test OK.
- **Doğrulama:** makemigrations --check ✔ (değişiklik yok), migrate --check ✔, `python manage.py test bookings shuttles` ✔ (78 OK), `tsc --noEmit` ✔, `npm run lint` ✔, `npm run build` ✔, AST duplicate-method taraması ✔ temiz.
...
### [x] F5-07 · İlk yükleme ve performans
...
**Notlar:** İki mimari karar kullanıcıyla netleştirildi (bkz. aşağı): **(1)** ISR, F3-05'te giren istek-başına CSP nonce'unu gevşetmeden mümkün değil → **CSP korundu, ISR ertelendi**; bunun yerine algılanan-performans kazanımları teslim edildi. **(2)** `next.config.ts` `images.unoptimized:true` → `next/image` optimizasyon yapmıyor; **deploy güvenliği için `unoptimized` korundu**, `<img>`'ler yine de `next/image`'e taşındı (lazy-load + boyut/CLS kazancı).
- **LCP kazancı (hero):** `app/page.tsx` hero başlığı/alt başlığı `mounted ? ... : ''` ile ilk boyamada BOŞ kalıyordu (H1 = LCP elemanı boş → geç LCP). `LocaleProvider` locale'i deterministik `tr-TR` ile başlatıp localStorage'ı yalnız `useEffect`'te okuduğu için sunucu ve ilk istemci render'ı zaten aynı (`suppressHydrationWarning` mevcut) → gate gereksizdi. Kaldırıldı; hero artık ilk boyamada dolu. Kullanılmayan `mounted` state+effect de silindi.
- **Combos skeleton:** ana sayfadaki tek gerçek async bölüm (`/combos/`) için `combosLoading` bayrağı + animate-pulse skeleton kartları eklendi (önce yüklenene kadar bölüm hiç görünmüyordu).
- **`<img>` → `next/image`:** payment logoları local SVG'ye (`public/payments/visa.svg`, `mastercard.svg`) taşındı (harici Wikimedia bağımlılığı kaldırıldı); checkout combo/shuttle/tur thumbnail'ları, restoran logosu, admin başvuru logosu, group-chat galeri thumbnail'ı `next/image` (`fill`+`sizes`) oldu. **Bilerek `<img>` bırakılanlar:** data-URL kaynaklar (QR/harita/dosya önizleme: offline-tickets, OfflineGuard, SecureFileUpload, post-tour-review), boyutu bilinmeyen chat balonu medyası (group-chat:253) ve özel durumlar (`GlobalImageFallback` global hata dinleyici, `api/og` `ImageResponse`).
- **Doğrulama:** `npx tsc --noEmit` temiz; `npm run lint` temiz; `npm run build` başarılı.
- **Ölçülemeyen/ertelenen:** Lighthouse mobil LCP ölçümü bu ortamda çalışan sunucu + lighthouse olmadığı için alınamadı (insan aksiyonu). ISR ile gerçek statik LCP kazancı ve `next/image` optimizasyonu CSP/deploy kararlarına bağlı — bkz. BULUNAN YENİ SORUNLAR.
...
* ~~**[P0 · para] Transfer (shuttle) akışında overbooking yarışı duruyor.**~~ **ÇÖZÜLDÜ (F5-01):** `_create_shuttle_booking` artık kontenjanı tur akışıyla (F2-02) birebir aynı desende **`create()` anında koşullu atomik UPDATE** ile tutuyor (`booked_count__lte=F('max_capacity')-guests`; kaybeden istek 0 satır günceller → 400). Webhook `SUCCEEDED` artık transfer kontenjanına dokunmuyor (çift sayım kalktı), yalnız durumu `confirmed` yapıyor; webhook `FAILED` ve `cancel` (pending+confirmed) yeni `_release_capacity` dispatcher'ıyla türüne göre kontenjanı bırakıyor. `bookings/tests.py::ShuttleCapacityReservationTestCase` (create-anı rezervasyon, overbooking reddi, pending iptal + başarısız ödeme bırakma) ve güncellenen `test_shuttle_webhook_never_touches_quota` regresyonu kilitliyor.
