    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col md:flex-row pb-20 md:pb-0">
      ...
      <div className="md:hidden bg-white/90 dark:bg-slate-900/90 backdrop-blur-md shadow-sm border-b border-slate-200 dark:border-slate-800 p-4 sticky top-0 z-30 flex justify-between items-center">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold text-slate-900 dark:text-white font-sans">Acenta Paneli</h1>
            <p className="text-[10px] text-slate-500 uppercase tracking-widest font-semibold flex items-center gap-1 font-sans">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-72 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 flex-col shadow-2xl transform transition-transform duration-300 md:relative md:translate-x-0 md:flex ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-6 border-b border-slate-200 dark:border-slate-800 flex flex-col gap-4">
          <h2 className="text-xl font-bold text-slate-900 dark:text-white tracking-tight font-sans">İşletme Merkezi</h2>
          <div className="flex items-center gap-3 bg-slate-50 dark:bg-slate-800/50 p-3 rounded-lg border border-slate-100 dark:border-slate-800">
            ...
            <div className="flex flex-col">
              <span className="text-sm font-semibold text-slate-900 dark:text-white leading-tight font-sans">{user?.username || 'Acenta'}</span>
              <span className="text-[11px] text-slate-500 flex items-center gap-1.5 font-medium mt-0.5"><span className="w-1.5 h-1.5 rounded-full bg-green-500"></span> Onaylı Partner</span>
            </div>
          </div>
        </div>
        <nav className="flex-1 px-4 py-6 space-y-1.5 overflow-y-auto">
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                <span className={`text-lg opacity-80 ${isActive ? 'scale-105 opacity-100' : ''} transition-transform`}>{item.icon}</span>
                <span>{item.name}</span>
              </Link>
            )
          })}
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white/90 dark:bg-slate-900/90 backdrop-blur-lg border-t border-slate-200 dark:border-slate-800 px-2 py-2 flex justify-between items-center z-40 pb-safe shadow-[0_-4px_10px_rgba(0,0,0,0.02)] overflow-x-auto">
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              <div className={`text-xl ${isActive ? 'scale-110 mb-0.5' : ''} transition-all duration-200 opacity-80`}>
                {item.icon}
              </div>
              <span className="text-[10px] font-medium tracking-tight text-center font-sans">{item.name}</span>
            </Link>
          )
        })}
      </div>
